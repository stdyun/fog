  ...
  module Compute
    class Ecloud
      class Organization < Fog::Ecloud::Model
        ...
        def environments
          @environments ||= Fog::Compute::Ecloud::Environments.new(:service => service, :href => href)
        end

        def tags
          @tags ||= Fog::Compute::Ecloud::Tags.new(:service => service, :href => "/cloudapi/ecloud/deviceTags/organizations/#{id}")
        end

        def admin
          @admin ||= Fog::Compute::Ecloud::AdminOrganizations.new(:service => service, :href => "/cloudapi/ecloud/admin/organizations/#{id}").first
        end

        def users
          @users ||= Fog::Compute::Ecloud::Users.new(:service => service, :href => "/cloudapi/ecloud/admin/users/organizations/#{id}")
        end
