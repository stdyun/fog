  ...
  module Compute
    class Ecloud
      class Environment < Fog::Ecloud::Model
        ...
        def networks
          @networks ||= Fog::Compute::Ecloud::Networks.new(:service => service, :href => "/cloudapi/ecloud/networks/environments/#{id}")
        end
        ...
        def layout
          @layout ||= Fog::Compute::Ecloud::Layouts.new(:service => service, :href => "/cloudapi/ecloud/layout/environments/#{id}").first
        end
        ...
        def catalog
          ...
          @catalog ||= Fog::Compute::Ecloud::Catalog.new(:service => service, :href => "/cloudapi/ecloud/admin/catalog/organizations/#{org_href.scan(/\d+/)[0]}")
        end
